  ...
  return (
    <div className="shrink-0 p-3 border-t border-border bg-card/80 backdrop-blur-xl">
      <div className="flex items-end gap-2 max-w-3xl mx-auto">
        <input type="file" ref={fileInputRef} className="hidden" accept=".pdf,.docx,.txt,.md,.csv,.png,.jpg,.jpeg,.webp,.bmp" onChange={onFileUpload} />
        <button onClick={() => fileInputRef.current?.click()} className="p-2.5 rounded-xl hover:bg-secondary transition-colors text-muted-foreground hover:text-foreground shrink-0" title="Upload PDF, DOCX, image — OCR auto-runs on scanned pages">
